...
export default {
  ...
  setup() {
    ...
    const load = async (page = 1) => {
      const response = await axios.get(`${process.env.VUE_APP_ADMIN_URL_PATH_ADMIN}/users?page=${page}`);
    ...
    const del = async (id: number) => {
      if (confirm("Are you sure you want to delete this record?")) {
        await axios.delete(`${process.env.VUE_APP_ADMIN_URL_PATH_ADMIN}/users/${id}`);
